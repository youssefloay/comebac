// Script pour uploader Mangoz FC.jpg vers santos-xxx.jpg
// Car le fichier Mangoz FC.jpg contient l'image Santos
// Usage: npx tsx scripts/upload-santos-from-mangoz-file.ts
...
async function uploadSantos() {
  console.log('📤 Upload de Mangoz FC.jpg vers santos-xxx.jpg...')
  console.log('⚠️  Le fichier Mangoz FC.jpg contient l\'image Santos\n')

  const assetsDir = resolve(process.cwd(), 'assets')
  const mangozLocalFile = path.join(assetsDir, 'Mangoz FC.jpg') // Ce fichier contient l'image Santos
  ...
  console.log(`📄 Fichier local: Mangoz FC.jpg (contient l'image Santos)`)
  console.log(`📄 Fichier Storage cible: santos-${santos.id}.jpg\n`)

  try {
    // Uploader l'image
    console.log('📤 Upload de l\'image...')
    const santosUrl = await uploadJerseyImage(santos.id, santos.name, mangozLocalFile)
    ...
    console.log('\n✅ Terminé avec succès!')
    console.log(`📄 Le fichier santos-${santos.id}.jpg contient maintenant la bonne image Santos`)
