...
async function checkRoadToGloryPlayers() {
  console.log('🔍 Vérification du nombre de joueurs pour "Road To Glory"...\n')
  ...
  if (!teamsSnap.empty) {
    ...
    console.log(`   📊 Nombre de joueurs dans playerAccounts avec teamId="${teamId}": ${playerAccountsSnap.size}`)
    
    if (playerAccountsSnap.size > 0) {
      console.log(`   📝 Liste des joueurs:`)
      playerAccountsSnap.forEach((doc, index) => {
        const data = doc.data()
        console.log(`      ${index + 1}. ${data.firstName} ${data.lastName} (${data.email || 'N/A'}) - #${data.jerseyNumber || data.number || 'N/A'}`)
      })
    }
  } else {
    const playerAccountsByNameSnap = await db.collection('playerAccounts')
      .where('teamName', '==', teamName)
      .get()
    
    console.log(`   📊 Nombre de joueurs dans playerAccounts avec teamName="Road To Glory": ${playerAccountsByNameSnap.size}`)
  }
  console.log('')
  ...
  if (!teamsSnap.empty) {
    ...
    console.log(`   📊 Nombre de joueurs dans players avec teamName="Road To Glory": ${playersByNameSnap.size}`)
  }
  console.log('')
