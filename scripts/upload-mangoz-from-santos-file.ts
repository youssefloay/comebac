// Script pour uploader Santos FC.jpg vers mangoz-xxx.jpg
// Car le fichier Santos FC.jpg contient l'image Mangoz
// Usage: npx tsx scripts/upload-mangoz-from-santos-file.ts
...
async function uploadMangoz() {
  console.log('📤 Upload de Santos FC.jpg vers mangoz-xxx.jpg...')
  console.log('⚠️  Le fichier Santos FC.jpg contient l\'image Mangoz\n')

  const assetsDir = resolve(process.cwd(), 'assets')
  const santosLocalFile = path.join(assetsDir, 'Santos FC.jpg') // Ce fichier contient l'image Mangoz
  ...
  console.log(`📄 Fichier local: Santos FC.jpg (contient l'image Mangoz)`)
  console.log(`📄 Fichier Storage cible: mangoz-fc-${mangoz.id}.jpg\n`)

  try {
    // Uploader l'image
    console.log('📤 Upload de l\'image...')
    const mangozUrl = await uploadJerseyImage(mangoz.id, mangoz.name, santosLocalFile)
    ...
    console.log('\n✅ Terminé avec succès!')
    console.log(`📄 Le fichier mangoz-fc-${mangoz.id}.jpg contient maintenant la bonne image Mangoz`)
